...
const SuperadminDashboard = () => {
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 to-gray-800 p-6 font-sans overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        <motion.h1 
          ...
          className="text-5xl font-extrabold text-center text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-purple-500 mb-12 pt-8 tracking-tight"
        >
          Superadmin <span className="text-white">Command Center</span>
        </motion.h1>

        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {features.map(({ title, description, link, color, icon, pattern, hover }, index) => (
            <motion.div
              key={title}
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
            ...
            >
              <Link
                to={link}
                className={`relative overflow-hidden rounded-2xl p-6 h-full flex flex-col ${color} ${hover} transition-all duration-300 ease-[cubic-bezier(0.4,0,0.2,1)] shadow-lg group`}
              >
                {/* Pattern overlay */}
        ...
        <motion.div 
          ...
          whileTap={{ scale: 0.95 }}
        >
          <button className="p-4 bg-gradient-to-br from-pink-500 to-rose-600 rounded-full shadow-xl shadow-pink-500/20 text-white flex items-center justify-center">
