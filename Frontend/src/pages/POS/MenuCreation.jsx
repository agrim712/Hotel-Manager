...
const MenuCreationContent = () => {
  ...
  useEffect(() => {
    fetchMenuData();
  }, [selectedOutlet]); // Refetch when outlet changes
  ...
  const fetchMenuData = useCallback(async () => {
    try {
      ...
      // Build query parameters for outlet filtering
      const outletParams = selectedOutlet ? `?outletId=${selectedOutlet.id}` : '';

      const [categoriesResponse, itemsResponse, modifiersResponse, combosResponse] = await Promise.all([
        axios.get(`${API_URL}/categories${outletParams}`, { headers }),
        axios.get(`${API_URL}/items${outletParams}`, { headers }),
        axios.get(`${API_URL}/modifiers${outletParams}`, { headers }),
        axios.get(`${API_URL}/combos${outletParams}`, { headers }),
      ]);
  ...
  const handleCreateOrUpdateItem = async (e) => {
    ...
    // Include outletId so items are linked to the selected outlet
    if (selectedOutlet?.id) {
      formData.append('outletId', selectedOutlet.id);
    }
  ...
  if (loading && categories.length === 0) {
    return (
      ...
      </div> // <-- MISSING CLOSING DIV ADDED HERE (Line 434)
    );
  }
  ...
  if (error) {
    return (
      ...
      </div> // <-- MISSING CLOSING DIV ADDED HERE (Line 454)
    );
  }
